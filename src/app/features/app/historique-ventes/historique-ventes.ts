...
export class HistoriqueVentesComponent implements OnInit {
  ...
  readonly cashier    = (s: any)    =>
    s.cashier ? `${s.cashier.first_name} ${s.cashier.last_name}` : '—';
  ...
  readonly filteredTotal = computed(() =>
    this.sales().reduce((sum, s) => sum + parseFloat(s.total_amount ?? 0), 0)
  );
  ...
  load() {
    ...
    this.salesService.getAll(this.buildParams(this.dateRange(this.activeFilter()))).subscribe({
      next: (res) => {
  ...
  private loadToday() {
    this.salesService.getAll(this.buildParams(this.dateRange('today'))).subscribe({
